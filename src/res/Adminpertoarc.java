 ...
 */
@WebServlet("/Adminpertoarc")
public class Adminpertoarc extends HttpServlet {
	...
	//	String id = request.getParameter("id");
		...
		//	String s = "INSERT INTO clientregform1 SELECT * FROM clientregform WHERE UserName='"+UserName+"' ";
			String s1 = "update plandata set status='"+st+"' where filename='"+filename+"'";
	...
	//		int r3 = DB.getConnection().prepareStatement(s3).executeUpdate();
			...
			if(r1>0)
				...
				{
					out.print("<html><body><script>alert(' Architech Plan Accepted');</script></body></html>");
					RequestDispatcher rs1 = request.getRequestDispatcher("adminviewplan.jsp");
					rs1.include(request, response);
				}
				else 
				{
					out.print("<html><body><script>alert(' Registration Accepted');</script></body></html>");
					RequestDispatcher rs1 = request.getRequestDispatcher("adminviewplan.jsp");
					rs1.include(request, response);
				}
